...
const menuItems = [
  { name: "Balık Ekmek", note: "Günün balığı, çıtır ekmek ve yeşillik", image: "/images/food-sandwich.jpg" },
  { name: "Günün Balığı", note: "Tezgahtan seçin, dilediğiniz gibi pişirelim", image: "/images/food-fish.jpg" },
  ...
  return (
    <main>
      <section className="hero">
        <Image className="hero-image" data-parallax src="/images/view.jpg" alt="Galata Köprüsü altında, denizin kıyısındaki Cansın Restaurant" fill sizes="100vw" priority />
        <div className="hero-shade" />
        <div className="hero-copy">
          <p className="eyebrow" data-hero-reveal>İstanbul · Eminönü</p>
          <h1 data-hero-reveal>Denizin üzerinde,<br /><em>İstanbul’un içinde.</em></h1>
      ...
      <section className="story" data-reveal>
        ...
        <div className="story-copy" data-reveal-item><p className="eyebrow dark">Cansın’ın hikâyesi</p><h2>Köprüden geçenlerin değil,<br /><em>köprüde kalanların</em> sofrası.</h2><p>Vapur sesleri, martılar, İstanbul’un iki yakası… Cansın’da yemek yalnızca yemek değildir. Şehrin tam kalbinde, denize birkaç adım mesafede paylaşılan bir İstanbul hatırasıdır.</p><Link className="text-link" href="/hakkimizda">Hikâyemizi okuyun <ArrowIcon /></Link></div>
        <div className="story-visual parallax-frame" data-reveal-item><Image data-parallax src="/images/hero.jpg" alt="Cansın Restaurant iç mekânı" fill sizes="(max-width: 800px) 100vw, 38vw" /></div>
      </section>

      <section className="view-break parallax-frame" data-reveal>
        <Image data-parallax src="/images/terrace.jpg" alt="Haliç kıyısında Cansın balık ekmek sofrası" fill sizes="100vw" />
      ...
      <section className="flavours" data-reveal>
        <div className="section-heading" data-reveal-item><p className="eyebrow dark">Tezgahtan sofraya</p><h2>Deniz ne verdiyse,<br /><em>usta onu güzelleştirir.</em></h2><Link className="text-link" href="/lezzetler">Tüm lezzetler <ArrowIcon /></Link></div>
      ...
      <section className="gallery-section" data-reveal>
        <div className="gallery-heading" data-reveal-item><div><p className="eyebrow dark">Cansın’dan kareler</p><h2>Köprünün altında<br /><em>hayat var.</em></h2></div><a className="social-link" href="https://www.instagram.com/cansinrestaurant/" target="_blank" rel="noreferrer"><InstagramIcon /> Instagram’da takip et</a></div>
        <div className="gallery-grid">{gallery.map((image) => <figure className={`${image.wide ? "wide" : ""} ${image.tall ? "tall" : ""}`} data-reveal-item key={image.src}><Image src={image.src} alt={image.alt} fill loading="eager" sizes="(max-width: 800px) 100vw, 50vw" /></figure>)}</div>
